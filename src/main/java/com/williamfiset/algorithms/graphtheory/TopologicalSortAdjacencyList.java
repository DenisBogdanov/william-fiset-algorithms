...
public class TopologicalSortAdjacencyList {
  ...
  private static int dfs(int indexToInsertNode, int at, boolean[] visited, int[] ordering,
                         Map<Integer, List<Edge>> graph) {

    visited[at] = true;

    for (Edge edge : graph.getOrDefault(at, List.of())) {
      if (!visited[edge.to]) {
        indexToInsertNode = dfs(indexToInsertNode, edge.to, visited, ordering, graph);
      }
    }
    ordering[indexToInsertNode] = at;
    return indexToInsertNode - 1;
  }
  ...
  //
  // NOTE: 'nodeSize' is not necessarily the number of nodes currently present
  ...
  //
  public static int[] topologicalSort(Map<Integer, List<Edge>> graph, int nodeSize) {
    int[] ordering = new int[nodeSize];
    boolean[] visited = new boolean[nodeSize];

    int indexToInsertNode = nodeSize - 1;
    for (int at = 0; at < nodeSize; at++) {
      if (!visited[at]) {
        indexToInsertNode = dfs(indexToInsertNode, at, visited, ordering, graph);
      }
    }
  ...
  // wouldn't be present in the adjacency list but are still part of the graph!
  //
  public static Integer[] dagShortestPath(Map<Integer, List<Edge>> graph,
                                          int start, int numNodes) {
    ...
    for (int i = 0; i < numNodes; i++) {
      ...
      if (dist[nodeIndex] != null) {
        for (Edge edge : graph.getOrDefault(nodeIndex, List.of())) {
